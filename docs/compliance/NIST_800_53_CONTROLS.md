...
| Control | Title | Status | Implementation | Evidence |
|---------|-------|--------|----------------|----------|
| **CP-9** | System Backup | Aligned | DynamoDB Point-in-Time Recovery enabled. S3 versioning on all buckets. Terraform state backed up in S3 with versioning. Optional cross-region replication module (`terraform/modules/replication`) can replicate `files`, `processed`, and CloudTrail buckets to a passive secondary region for DR exercises, but is disabled by default in the cost-aware thesis baseline. | `terraform/modules/storage/main.tf` — PITR, versioning, `terraform/modules/replication/main.tf` |
| **CP-10** | System Recovery | Aligned | Single-region `us-west-2` baseline with Pilot Light rebuild from Terraform, versioned state, signed ECR image digests, S3 versioning, and DynamoDB PITR. Optional CRR reduces S3 RPO when explicitly enabled for a DR validation run. | `docs/DISASTER_RECOVERY.md`, `terraform/modules/replication/main.tf`, `.github/workflows/deploy-environment.yml` |
...
| **SC-28(1)** | Cryptographic Protection (at rest) | Aligned | Customer-managed KMS key. Key tagged `Compliance = FIPS-140-3-Oriented`. Deletion window: 7 days minimum. When optional replication is enabled, replica buckets are encrypted with a dedicated regional KMS key (replication module sets `kms_key_id` per destination). | `terraform/modules/security/main.tf` — `aws_kms_key`, `terraform/modules/replication/main.tf` |
...
| Control | Title | Status | Implementation | Evidence |
|---------|-------|--------|----------------|----------|
| **SR-3** | Supply Chain Controls and Processes | Aligned | Container images are signed with cosign keyless (Sigstore/Fulcio OIDC), SBOM/provenance metadata is produced during Docker builds, and deploy verifies signatures before Terraform applies immutable `repo@sha256` image references. ECR enables `ENHANCED` continuous vulnerability scanning (Inspector). | `fsamp-code-ci/.github/actions/docker-build/action.yml`, `.github/workflows/deploy-environment.yml`, `terraform/modules/ecr/main.tf` |
| **SR-4** | Provenance | Aligned | Cosign signatures preserve OIDC certificate identity (workflow + repository), enabling deploy-time verification that the image digest was produced by an allowed GitHub workflow before promotion. | `.github/workflows/deploy-environment.yml` — `cosign verify`, `--certificate-identity-regexp` |
